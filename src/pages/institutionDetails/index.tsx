...
export const InstitutionDetails = () => {
  ...
  const [listDonations, setListDonations] = useState<iDonations[]>(
    [] as iDonations[]
  );
  const [institution, setInstitution] = useState<iInstitution[]>(
    [] as iInstitution[]
  );
  const [verifyUserSubscribe, setVerifyUserSubscribe] = useState<iSubscribe[]>(
    [] as iSubscribe[]
  );
    ...
    <Container>
      ...
      {listEvents.map((event) => {
        return (
          <section>
            ...
            <p>
              Quando: {event.date} às {event.hour}
            </p>
      ...
      {listDonations.map((donation) => {
        return (
          <section>
            <p>
              <span>{donation.quantity}</span> {donation.name}
            </p>
            <ProgressBar
              width={"200px"}
              completed={(
